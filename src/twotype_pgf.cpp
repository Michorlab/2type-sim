                           ...
                           int ancestors = 1, int threads = 1)
{
  ...
  for(int l = 0; l < (dom*dom); l++)
  {
    ...
    std::complex<double> s1_exp = R * exp(std::complex<double>(2 * M_PI * j,0) * std::complex<double>(0,1) / dom);
    std::complex<double> s2_exp = R * exp(std::complex<double>(2 * M_PI * k,0) * std::complex<double>(0,1) / dom);
    std::complex<double> num_sol = pow(A_gf_2(s1_exp, s2_exp, t, alpha1, alpha2, lambda2, omega, a, b, c), ancestors);
    freq_dat[l] =  num_sol;
  }
...
{
  std::complex<double> z_ = z(y, t, alpha2, lambda2);
  std::complex<double> C1_ = C1_2(x, y, t, alpha1, alpha2, lambda2, omega, a, b, c);
  return 1.0 + omega / alpha1 + lambda2 / alpha1 *
